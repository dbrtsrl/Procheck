...
PLANS_FILE = str(APP_DATA_DIR / "plans_by_date.json")
...
class StatsEngine:
    ...
    def sessions_by_task_in_range(self, start_day: str, end_day: str):
        """
        Aggregate sessions by task for the given day range.

        Definitions:
        - COMPLETED session: event == 'session_complete' (this is what counts as "done").
        - EARLY stop: event == 'stop_early' (stopped manually before completion).
        - CANCEL: event == 'cancel' (kept for safety, but not used in summary KPIs).
        """
        evs = self.events_in_range(start_day, end_day)
        ...
        for e in evs:
            evt = e.get("event")
            if evt not in ("session_complete", "stop_early", "cancel", "stop"):
                continue

            task = (e.get("task") or "").strip() or "(no task)"
            rec = by.setdefault(task, {"sessions": 0, "total_secs": 0, "early": 0})

            # Backward compatibility: old logs may use 'stop' for completed sessions.
            if evt in ("session_complete", "stop"):
                rec["sessions"] += 1
                try:
                ...
                rec["early"] += 1
            # 'cancel' intentionally ignored here
        ...
        for task, rec in by.items():
            ...
            rows.append((task, sessions, total_secs, avg, int(rec.get("early", 0) or 0)))
        rows.sort(key=lambda r: (r[2], r[1]), reverse=True)
        return rows

    def session_stats_in_range(self, start_day: str, end_day: str):
        """
        Return (avg_completed_session_secs, early_stop_rate, avg_time_to_first_procrastination_secs)
        in a date range.

        Definitions:
        - Completed session: 'session_complete' (and legacy 'stop').
        - Early stop: 'stop_early'.
        - Cancel: ignored for these KPIs (used only to abort accidental sessions).
        """
        evs = self.events_in_range(start_day, end_day)

        completed = [e for e in evs if e.get("event") in ("session_complete", "stop")]
        early = [e for e in evs if e.get("event") == "stop_early"]

        if completed:
            avg_secs = sum(max(0, int(e.get("secs", 0) or 0)) for e in completed) / max(1, len(completed))
        else:
            avg_secs = None

        denom = (len(completed) + len(early))
        early_stop_rate = (len(early) / denom) if denom else None
        ...
        while i < len(evs):
            ...
            j = i + 1
            first = None
            while j < len(evs) and evs[j].get("event") not in ("session_complete", "stop", "stop_early", "cancel", "start"):
        ...
        return avg_secs, early_stop_rate, avg_tt
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.remaining_text = tk.StringVar(value="Remaining: 00:00:00")
        ...
        self.plans_map = self._load_plans()
        ...
        # Ensure the window is on-screen (in case a saved geometry is off-screen)
        try:
    ...
    def _ensure_window_visible(self):
        """Best-effort safeguard against off-screen window positions."""
        try:
            ...
            if w <= 1 or h <= 1 or sw <= 1 or sh <= 1:
                return

            margin = 40
            off_left = x < (-w + margin)
            off_top = y < (-h + margin)
            off_right = x > (sw - margin)
            off_bottom = y > (sh - margin)
    ...
    def _save_plans(self):
        try:
            with open(PLANS_FILE, "w", encoding="utf-8") as f:
                json.dump(self.plans_map, f, ensure_ascii=False, indent=2)
        except Exception:
            pass

    
    def _plans_for_day(self, ds: str):
        items = self.plans_map.get(ds, [])
        if isinstance(items, list):
            # Ensure list items are dicts
            return [it for it in items if isinstance(it, dict)]
        return []

    def _planner_week_range(self, anchor_ds: str):
        try:
            anchor = datetime.strptime(anchor_ds, "%Y-%m-%d").date()
        except Exception:
            anchor = date.today()
        wk_start = monday_of(anchor)
    ...
    def _done_minutes_by_task_in_range(self, start_day: str, end_day: str):
        by = {}
        for e in self.stats.events_in_range(start_day, end_day):
            if e.get("event") not in ("stop", "session_complete"):
                continue
            task = (e.get("task") or "").strip()
            if not task:
                continue
            try:
                secs = int(e.get("secs", 0) or 0)
            except Exception:
                secs = 0
            by[task] = by.get(task, 0) + max(0, secs)
        # convert to minutes (rounded to nearest minute)
        return {t: int(round(secs / 60.0)) for t, secs in by.items()}

    def _planned_minutes_in_range(self, start_day: str, end_day: str) -> int:
        try:
        ...
        while cur <= d1:
            ...
            for rec in self._plans_for_day(ds):
                try:
                    total += int(rec.get("planned_min", 0) or 0)
                except Exception:
                    pass
            cur += timedelta(days=1)
        return int(max(0, total))

    def _planner_remaining_minutes_in_range(self, start_day: str, end_day: str) -> int:
        """Planner-based remaining minutes for a day range.

        Remaining = planned minutes not yet covered by DONE minutes for the same task,
        with sequential allocation in planner order (same as carry-over).
        """
        try:
            self.stats.load()
        except Exception:
        ...
        done_by_task = self._done_minutes_by_task_in_range(start_day, end_day)
        ...
        while cur <= d1:
            ...
            for rec in self._plans_for_day(ds):
                ...
                if not task:
                    continue
                try:
                    pmin = int(rec.get("planned_min", 0) or 0)
                except Exception:
                    pmin = 0
                if pmin <= 0:
                    continue
                pool = int(done_by_task.get(task, 0) or 0)
                used = min(pmin, max(0, pool))
                done_by_task[task] = max(0, pool - used)
                remaining_total += max(0, pmin - used)
            cur += timedelta(days=1)

        return int(max(0, remaining_total))


    def _planned_tasks_in_range(self, start_day: str, end_day: str):
        try:
            d0 = datetime.strptime(start_day, "%Y-%m-%d").date()
            d1 = datetime.strptime(end_day, "%Y-%m-%d").date()
        except Exception:
    ...
    def _load_current_plan(self):
        if not hasattr(self, "plan_tv"):
            return

        ds = self.selected_day.get()
        view = (self.planner_view.get() if hasattr(self, "planner_view") else "Selected day")

        # Show/Hide day column based on view
        try:
            if view == "Selected day":
                self.plan_tv.column("day", width=0, stretch=False, minwidth=0)
                self.plan_tv.heading("day", text="")
            else:
                self.plan_tv.column("day", width=80, stretch=False, minwidth=70)
                self.plan_tv.heading("day", text="Day")
        except Exception:
            pass

        for it in self.plan_tv.get_children():
            self.plan_tv.delete(it)

        if view == "Selected week":
            start_s, end_s = self._planner_week_range(ds)
            try:
            ...
            while cur <= d1:
                ...
                items = self._plans_for_day(dkey)
                for i, rec in enumerate(items):
                    ...
                    try:
                        ...
                        pmin = 0
                    iid = f"p|{dkey}|{i}"
                    self.plan_tv.insert("", "end", iid=iid, values=(dkey[5:], task, pmin if pmin else ""))
                cur += timedelta(days=1)
        else:
            items = self._plans_for_day(ds)
            for i, rec in enumerate(items):
                task = (rec.get("task") or "").strip()
                try:
                    pmin = int(rec.get("planned_min", 0) or 0)
                    ...
                    pmin = 0
                iid = f"p|{ds}|{i}"
                self.plan_tv.insert("", "end", iid=iid, values=("", task, pmin if pmin else ""))


        # Planner total (minutes/hours) for current view
        try:
            if hasattr(self, "plan_total_lbl"):
                if view == "Selected week":
                    start_s, end_s = self._planner_week_range(ds)
                    total_m = self._planned_minutes_in_range(start_s, end_s)
                    self.plan_total_lbl.config(text=f"Planned: {self._fmt_min_hm(total_m)}")
                else:
                    total_m = self._planned_minutes_in_range(ds, ds)
                    self.plan_total_lbl.config(text=f"Planned: {self._fmt_min_hm(total_m)}")
        except Exception:
            pass

        # Planner remaining (planned minus done, sequential allocation)
        try:
            if hasattr(self, "plan_remaining_lbl"):
                if view == "Selected week":
                    start_s, end_s = self._planner_week_range(ds)
                    rem_m = self._planner_remaining_minutes_in_range(start_s, end_s)
                    self.plan_remaining_lbl.config(text=f"Remaining: {self._fmt_min_hm(rem_m)}")
                else:
                    rem_m = self._planner_remaining_minutes_in_range(ds, ds)
                    self.plan_remaining_lbl.config(text=f"Remaining: {self._fmt_min_hm(rem_m)}")
        except Exception:
            pass


        # Carry-over button: day view -> next day, week view -> next week
        try:
            if hasattr(self, "plan_carry_btn"):
                if view == "Selected week":
                    self.plan_carry_btn.configure(text="Carry over → next week", state="normal")
                else:
                    self.plan_carry_btn.configure(text="Carry over → next day", state="normal")
        except Exception:
            pass

    def _add_planned_task(self):
        ds = self.selected_day.get()
        raw = simpledialog.askstring(
            "Planned task",
            "Enter task + planned minutes in ONE prompt. Examples:\n\n"
            "  Deep work | 60\n"
            "  Anki 45\n"
            "  Reading; 90\n\n"
            "Minutes must be 5–600.",
            parent=self,
        )
        if raw is None:
            return
        ...
        # Parse: accept separators (|, ;, ,) OR "<task> <minutes>" (last token numeric)
        task = ""
        pmin = None
        m = re.match(r"^(.*?)(?:\s*[\|;,]\s*)(\d{1,4})\s*$", raw)
        if m:
        ...
        if not task or pmin is None:
            messagebox.showerror("Planner", "Could not parse. Use e.g. 'Deep work | 60' or 'Deep work 60'.")
            return
        pmin = int(clamp(int(pmin), 5, 600))

        items = self._plans_for_day(ds)
        items.append({"task": task, "planned_min": pmin})
        self.plans_map[ds] = items
        self._save_plans()
        self._load_current_plan()
        self._request_refresh()

    def _delete_planned_task(self):
        if not hasattr(self, "plan_tv"):
            return
        sel = self.plan_tv.selection()
        if not sel:
            return

        # Each iid is p|YYYY-MM-DD|idx
        to_delete = []
        for s in sel:
            parts = str(s).split("|")
            if len(parts) != 3:
                continue
            _p, day_s, idx_s = parts
            try:
                idx = int(idx_s)
            except Exception:
                continue
            to_delete.append((day_s, idx))

        if not to_delete:
            return

        # delete per day, reverse index order
        by_day = {}
        for day_s, idx in to_delete:
            by_day.setdefault(day_s, []).append(idx)

        for day_s, idxs in by_day.items():
            items = self._plans_for_day(day_s)
            for i in sorted(set(idxs), reverse=True):
                if 0 <= i < len(items):
                    items.pop(i)
            if items:
                self.plans_map[day_s] = items
            else:
                self.plans_map.pop(day_s, None)
    ...
    def _carry_over_unfinished(self):
        """Carry over unfinished planned minutes.

        Rules:
        - We only carry PLANNED minutes that were not matched by DONE minutes for the same task.
        - DONE minutes are computed from logged completed work sessions (stop/session_complete).
        - Allocation is sequential in planner order (earlier planned items consume done minutes first).
        - Day view carries remaining to the NEXT DAY (ds + 1).
        - Week view carries remaining to the SAME WEEKDAY NEXT WEEK (ds + 7).
        """
        # Ensure stats are fresh for done-minute calculations.
        try:
            self.stats.load()
        except Exception:
            pass
        ...
        if view == "Selected week":
            start_s, end_s = self._planner_week_range(ds)
            # Build per-task done minutes pool for the week
            done_by_task = self._done_minutes_by_task_in_range(start_s, end_s)

            # Iterate planned items in week order and compute remaining per item using the pool
            try:
                d0 = datetime.strptime(start_s, "%Y-%m-%d").date()
                d1 = datetime.strptime(end_s, "%Y-%m-%d").date()
            except Exception:
            ...
            carry = []  # tuples: (target_day, task, remaining_min)
            cur = d0
            while cur <= d1:
                ...
                for rec in items:
                    ...
                    rem = max(0, pmin - used)
                    if rem > 0:
                        carry_day = (cur + timedelta(days=7)).strftime("%Y-%m-%d")
                        carry.append((carry_day, task, rem))
                cur += timedelta(days=1)

            if not carry:
                messagebox.showinfo("Carry over", "Nothing to carry over for the selected week.")
                return

            for day_s, task, rem in carry:
                items = self._plans_for_day(day_s)
                items.append({"task": task, "planned_min": int(rem)})
                self.plans_map[day_s] = items

        else:
            # Selected day: carry to next day
            start_s = end_s = ds
            done_by_task = self._done_minutes_by_task_in_range(start_s, end_s)
            carry_day = (datetime.strptime(ds, "%Y-%m-%d").date() + timedelta(days=1)).strftime("%Y-%m-%d")
            ...
            for rec in items:
                ...
                rem = max(0, pmin - used)
                if rem > 0:
                    carry.append((task, rem))

            if not carry:
                messagebox.showinfo("Carry over", "Nothing to carry over for the selected day.")
                return

            next_items = self._plans_for_day(carry_day)
            for task, rem in carry:
                next_items.append({"task": task, "planned_min": int(rem)})
            self.plans_map[carry_day] = next_items

        self._save_plans()
        self._load_current_plan()
        self._request_refresh()

    def _planned_count_in_range(self, start_day: str, end_day: str) -> int:
        # Backwards-compatible: number of planned entries (regardless of minutes)
        try:
            d0 = datetime.strptime(start_day, "%Y-%m-%d").date()
            d1 = datetime.strptime(end_day, "%Y-%m-%d").date()
        except Exception:
        ...
        while cur <= d1:
            ds = cur.strftime("%Y-%m-%d")
            total += len(self._plans_for_day(ds))
            cur += timedelta(days=1)
        return int(max(0, total))

    def _planned_days_in_range(self, start_day: str, end_day: str) -> int:
        # Days that have at least one planned entry
        try:
            d0 = datetime.strptime(start_day, "%Y-%m-%d").date()
            d1 = datetime.strptime(end_day, "%Y-%m-%d").date()
        except Exception:
        ...
        while cur <= d1:
            ds = cur.strftime("%Y-%m-%d")
            if len(self._plans_for_day(ds)) > 0:
                days += 1
            cur += timedelta(days=1)
        return int(max(0, days))


    def _completed_planned_items_in_range(self, start_day: str, end_day: str) -> int:
        """Count planner rows that are fully completed within the period.

        A planner row is considered completed if its planned minutes are fully covered by DONE minutes
        for the same task within the period. DONE minutes are allocated sequentially in planner order
        (same as carry-over allocation) to avoid double counting.
        """
        try:
            # Ensure fresh stats for accurate done-minute pooling
            self.stats.load()
        except Exception:
            pass
        ...
        while cur <= d1:
            ...
            for rec in self._plans_for_day(ds):
                ...
                if pool >= pmin:
                    completed += 1
                    done_by_task[task] = max(0, pool - pmin)
                else:
                    # not enough minutes to complete this row
                    done_by_task[task] = max(0, pool)
            cur += timedelta(days=1)

        return int(max(0, completed))

    def _fmt_min_hm(self, minutes: int) -> str:
        """Format minutes as a single unit (either minutes or hours), not both."""
        try:
            m = int(minutes or 0)
        except Exception:
            m = 0
        ...
        # Show minutes for < 60, otherwise show hours (integer if exact, else 1 decimal).
        if m < 60:
            return f"{m} min"
        if m % 60 == 0:
            return f"{m // 60} h"
    ...
    def _setup_style(self):
        ...
        style.configure("Break.TButton", padding=(14, 8), foreground="white", background="#1976d2")
        style.map("Break.TButton", background=[("active", "#1e88e5")])
    ...
    def _build_timer_tab(self):
        ...
        body = ttk.Frame(card)
        body.grid(row=0, column=0, sticky="nwe", padx=10, pady=10)
        body.columnconfigure(0, weight=1)

        # Task row (kept compact; extra space goes to spacer column)
        task_row = ttk.Frame(body)
        task_row.grid(row=0, column=0, sticky="we")
        task_row.columnconfigure(1, weight=0)
        task_row.columnconfigure(3, weight=1)

        ttk.Label(task_row, text="Task:", style="Title.TLabel").grid(row=0, column=0, sticky="w", pady=(0, 6))
        ttk.Entry(task_row, textvariable=self.active_task, width=32).grid(row=0, column=1, sticky="w", padx=(8, 8), pady=(0, 6))

        color_row = ttk.Frame(task_row)
        color_row.grid(row=0, column=2, sticky="w", pady=(0, 6))
        self.timer_task_color_swatch = tk.Canvas(color_row, width=18, height=18, highlightthickness=1, highlightbackground="#bbbbbb")
        self.timer_task_color_swatch.pack(side="left")
        self.timer_task_color_swatch.bind("<Button-1>", lambda _e: self.pick_task_color())
        ttk.Button(color_row, text="Color", style="Secondary.TButton", command=self.pick_task_color).pack(side="left", padx=(6, 0))

        # Spacer (so the task entry doesn't stretch across the whole card)
        ttk.Frame(task_row).grid(row=0, column=3, sticky="we")

        dur = ttk.LabelFrame(body, text="Target duration")
        dur.grid(row=1, column=0, sticky="we", pady=(6, 0))
        ...
        # Actions row: 3 columns

        # Actions row: single horizontal row
        actions = ttk.Frame(body)
        actions.grid(row=2, column=0, sticky="we", pady=(10, 0))

        for i in range(5):
            actions.columnconfigure(i, weight=1)

        ttk.Button(actions, text="Start", command=self.start_session, style="Start.TButton").grid(
            row=0, column=0, sticky="we", padx=4
        )
        ttk.Button(actions, text="Stop", command=self.stop_session, style="Stop.TButton").grid(
            row=0, column=1, sticky="we", padx=4
        )
        ttk.Button(actions, text="Mark procrastination", command=self.mark_procrastination, style="Procrastinate.TButton").grid(
            row=0, column=2, sticky="we", padx=4
        )
        ttk.Button(actions, text="Break", command=self.start_break, style="Break.TButton").grid(
            row=0, column=3, sticky="we", padx=4
        )
        ttk.Button(actions, text="Cancel", command=self.cancel_session, style="Cancel.TButton").grid(
            row=0, column=4, sticky="we", padx=4
        )
        ...
        self.timer_lbl.grid(row=1, column=0, sticky="w", padx=10, pady=(6, 0))

        # Remaining time line
        self.remaining_lbl = ttk.Label(info, textvariable=self.remaining_text, style="Status.TLabel")
        self.remaining_lbl.grid(row=2, column=0, sticky="w", padx=10, pady=(2, 6))

        self.progress = ttk.Progressbar(info, orient="horizontal", mode="determinate", style="Green.Horizontal.TProgressbar")
        self.progress.grid(row=3, column=0, sticky="we", padx=10, pady=(0, 10))
        ...
        ttk.Label(settings, text="Every (min):").grid(row=1, column=0, sticky="w", padx=10, pady=(0, 8))
        ttk.Spinbox(settings, from_=5, to=120, textvariable=self.current_interval_min, width=6).grid(row=1, column=1, sticky="w", padx=(10, 0), pady=(0, 8))
        ttk.Button(settings, text="Apply", command=self._manual_interval_changed).grid(row=1, column=2, sticky="w", padx=(10, 10), pady=(0, 8))
    ...
    def _build_stats_tab(self):
        ...
        planner_container = ttk.LabelFrame(self.left_vpw, text="Planner")
        planner_container.columnconfigure(0, weight=1)
        # rows: 0=view, 1=totals, 2=buttons, 3=table
        planner_container.rowconfigure(3, weight=1)

        # View mode: day / week
        view_row = ttk.Frame(planner_container)
        view_row.grid(row=0, column=0, sticky="we", padx=6, pady=(6, 2))
        view_row.columnconfigure(1, weight=1)

        ttk.Label(view_row, text="View:").grid(row=0, column=0, sticky="w")
        self.planner_view = tk.StringVar(value="Selected day")
        self.planner_view_cb = ttk.Combobox(
            view_row,
            textvariable=self.planner_view,
            values=["Selected day", "Selected week"],
            state="readonly",
            width=14
        )
        self.planner_view_cb.grid(row=0, column=1, sticky="w", padx=(6, 0))
        self.planner_view_cb.bind("<<ComboboxSelected>>", lambda _e: self._load_current_plan())

        # Totals stacked under view (keeps widget compact)
        totals_row = ttk.Frame(planner_container)
        totals_row.grid(row=1, column=0, sticky="we", padx=6, pady=(0, 4))
        totals_row.columnconfigure(0, weight=1)

        self.plan_total_lbl = ttk.Label(totals_row, text="")
        self.plan_total_lbl.grid(row=0, column=0, sticky="w")

        self.plan_remaining_lbl = ttk.Label(totals_row, text="")
        self.plan_remaining_lbl.grid(row=1, column=0, sticky="w")

        plan_btns = ttk.Frame(planner_container)
        plan_btns.grid(row=2, column=0, sticky="we", padx=6, pady=(2, 4))
        ttk.Button(plan_btns, text="Add", command=self._add_planned_task).pack(side="left")
        ttk.Button(plan_btns, text="Delete", command=self._delete_planned_task).pack(side="left", padx=(6, 0))
        self.plan_carry_btn = ttk.Button(plan_btns, text="Carry over", command=self._carry_over_unfinished)
        self.plan_carry_btn.pack(side="left", padx=(14, 0))

        plan_cols = ("day", "task", "min")
        self.plan_tv = ttk.Treeview(planner_container, columns=plan_cols, show="headings", height=10)
        self.plan_tv.heading("day", text="Day")
        self.plan_tv.heading("task", text="Task")
        self.plan_tv.heading("min", text="Planned min")
        self.plan_tv.column("day", width=80, anchor="w", stretch=False, minwidth=70)
        self.plan_tv.column("task", width=260, anchor="w", stretch=True, minwidth=120)
        self.plan_tv.column("min", width=90, anchor="e", stretch=False, minwidth=70)
        self.plan_tv.grid(row=3, column=0, sticky="nsew", padx=6, pady=(0, 6))

        plan_vsb = ttk.Scrollbar(planner_container, orient="vertical", command=self.plan_tv.yview)
        self.plan_tv.configure(yscrollcommand=plan_vsb.set)
        plan_vsb.grid(row=3, column=1, sticky="ns", pady=(0, 6))

        # Load planned tasks for the initially selected day/week
        self._load_current_plan()

# weights favor calendar heavily
        self.left_vpw.add(self.cal_container, weight=2)
        self.left_vpw.add(planner_container, weight=2)
        ...
        def _apply_initial_left_sash(_e=None):
            ...
            def _try_set(attempt=0):
                try:
                    ...
                    # Give Planner a bit more space while keeping Calendar fully visible
                    try:
                        cal_need = int(self.cal_container.winfo_reqheight() + 20)
                    except Exception:
                        cal_need = 0
                    pos = max(cal_need, int(h * 0.47))
                    self.left_vpw.sashpos(0, pos)
        ...
        # NOTE: internal event 'stop_early' is displayed as 'stop' in Today.
        ...
        self.kpi_meta = ttk.Label(summary_week, text="", justify="left", font=("Segoe UI", 9), wraplength=620)
        self.kpi_meta.grid(row=1, column=0, columnspan=2, sticky="we", padx=10, pady=(2, 6))

        # ON/OFF bar with percentage label
        self.summary_graph = tk.Canvas(summary_week, height=64, highlightthickness=0)
        self.summary_graph.grid(row=2, column=0, columnspan=2, sticky="we", padx=10, pady=(0, 8))
        ...
        week_row.grid(row=3, column=0, columnspan=2, sticky="we", padx=10, pady=(2, 4))
        ...
        self.week_lbl.grid(row=4, column=0, columnspan=2, sticky="w", padx=10, pady=(0, 4))

        self.week_graph = tk.Canvas(summary_week, height=78, highlightthickness=0)
        self.week_graph.grid(row=5, column=0, columnspan=2, sticky="we", padx=10, pady=(0, 10))
    ...
    def stop_session(self):
        ...
        tmin = self._target_minutes_int()
        interval_now = int(self.current_interval_min.get())
        self._append_log("stop_early", note=self._compose_note(None, tmin, interval_now))
    ...
    def _session_finished_dialog(self):
        ...
        # A completed (timer-expired) work block should be logged exactly once.
        # We log it as 'session_complete' regardless of the user's next choice,
        # so task aggregation remains correct and manual stops remain 'stop'.
        completed_secs = int(self.session_elapsed)
        if tmin and tmin > 0:
            completed_secs = int(tmin) * 60

        def _log_complete(choice: str):
            note = self._compose_note(None, tmin, interval_now, extra=f"auto_stop=1 choice={choice}")
            self._append_log("session_complete", note=note, secs_override=completed_secs, task_override=task)
    ...
    def _tick(self):
        if self.session_start_ts:
            ...
            try:
                ...
                target_s = 0
            if target_s > 0:
                remaining = max(0, target_s - int(self.session_elapsed))
                self.remaining_text.set(f"Remaining: {self._format_hms(remaining)}")
            else:
            ...
            self.progress["value"] = 0
            self.status.set("On break")
            self.remaining_text.set(f"Break: {self._format_hms(elapsed)}")

        if not self.session_start_ts and not self.on_break:
            self.remaining_text.set("Remaining: 00:00:00")
    ...
    def _update_day_and_week_views(self, ds: str):
        ...
        planned_items = self._planned_count_in_range(start_s, end_s) if hasattr(self, "plans_map") else 0
        planned_days = self._planned_days_in_range(start_s, end_s) if hasattr(self, "plans_map") else 0
        planned_min_total = self._planned_minutes_in_range(start_s, end_s) if hasattr(self, "plans_map") else 0
        planned_tasks = self._planned_tasks_in_range(start_s, end_s) if hasattr(self, "plans_map") else set()
        completed_items = self._completed_planned_items_in_range(start_s, end_s) if hasattr(self, "plans_map") else 0

        done_by_task = self._done_minutes_by_task_in_range(start_s, end_s) if hasattr(self, "plans_map") else {}
        done_min_planned = sum(int(done_by_task.get(t, 0) or 0) for t in planned_tasks) if planned_tasks else 0
        remaining_min = max(0, int(planned_min_total) - int(done_min_planned))
# total completed sessions in range (based on stop/session_complete)
        try:
            _sess_rows = self.stats.sessions_by_task_in_range(start_s, end_s)
            sessions_total = sum(int(r[1] or 0) for r in _sess_rows)
            total_session_secs = sum(int(r[2] or 0) for r in _sess_rows)
        except Exception:
            sessions_total = 0
        ...
        avg_secs, early_stop_rate, avg_tt = self.stats.session_stats_in_range(start_s, end_s)
        early_stops = 0
        try:
            early_stops = sum(1 for _e in self.stats.events_in_range(start_s, end_s) if _e.get('event') == 'stop_early')
        except Exception:
            early_stops = 0
        avg_s_txt = self._fmt_secs(avg_secs) if avg_secs is not None else "—"
        early_txt = (f"{int(early_stop_rate * 100)}%" if early_stop_rate is not None else "—")
        tt_txt = self._fmt_secs(avg_tt) if avg_tt is not None else "—"
        # KPI updates (percent is shown only on the visual bar)

        if hasattr(self, "kpi_meta"):
            # Keep Summary focused on plan completion (items) and behavior metrics.
            if planned_items > 0:
                comp_rate = int(round((completed_items / planned_items) * 100))
                items_line = f"Planned items: {planned_items}   |   Completed: {completed_items} ({comp_rate}%)"
            else:
                items_line = f"Planned items: {planned_items}   |   Completed: {completed_items}"

            line1 = f"{items_line}   |   Total session: {self._fmt_min_hm(int(round(total_session_secs / 60.0)))}   |   Sessions: {sessions_total}"
            line2 = f"Procs: {agg['procrastinations']}   |   Early stops: {early_stops}   |   Breaks: {agg.get('break_count',0)} ({self._fmt_secs(agg.get('break_secs',0))})"
            line3 = f"Avg session: {avg_s_txt}   |   Early stop rate: {early_txt}   |   Avg time to 1st proc: {tt_txt}"

            self.kpi_meta.config(text=(f"{title}\n{line1}\n{line2}\n{line3}"))
    ...
    def _display_event_name(self, evt: str):
        # Map internal events to user-facing labels in Today.
    ...
    def _apply_rundown_filters(self):
        ...
        allowed = {"start", "session_complete", "stop", "stop_early", "break", "cancel", "procrastination", "checkin_off_task"}
    ...
    def export_current_week_csv(self):
        ...
        try:
            with open(path, "w", encoding="utf-8", newline="") as f:
                ...
                w.writerow(["Early stop rate", "—" if early_stop_rate is None else f"{int(early_stop_rate*100)}%"])
...
if __name__ == "__main__":
    try:
        ...
        try:
            ...
            _p = Path(APP_DATA_DIR) / "startup_error.txt"
